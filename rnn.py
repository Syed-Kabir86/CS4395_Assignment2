...
if __name__ == "__main__":
    ...
    while not stopping_condition:
        ...
        if validation_accuracy > best_validation_accuracy:
            ...
            patience_counter += 1
            print(f'Validation accuracy didnt improve, patience counter is {patience_counter}')
            if patience_counter >= patience:
                stopping_condition = True
                print("Patience is done!")
                print("Best validation accuracy is:", best_validation_accuracy)

        # Commented this out incase I need this later
    ...
    # I reconfigured the validation code to test for the test.json file
    ...
    if args.test_data:
        ...
        test_data = [(x["text"].split(), int(x["stars"] - 1)) for x in testing]
        ...
        for input_words, gold_label in tqdm(test_data):
            ...
            input_words = input_words.translate(input_words.maketrans("", "", string.punctuation)).split()
            vectors = np.array([
                word_embedding.get(i.lower(), word_embedding['unk']) for i in input_words
            ])
            vectors = torch.tensor(vectors).view(len(vectors), 1, -1)
            output = model(vectors)
            predicted_label = torch.argmax(output)
